...
let stats = { activeVisitors: 0, chatsStarted: 0 };
...
io.on("connection", (socket) => {
  ...
  if (role === "admin") {
    ...
    socket.on("reset_stats", () => {
      stats = { activeVisitors: deviceConnections.size, chatsStarted: 0 };
  ...
  socket.on("visitor:service_selected", ({ service, sessionId: sid }) => {
    ...
    const greeting = service === 'General Consultation'
      ? `Hello! Thank you for reaching out. You\'re connected with Sarah R. Please describe your issue below.`
